...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-bd29d51b";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-bd29d51b">'+'<span class="tile-face-bd29d51b tile-front-bd29d51b"></span>'+'<span class="tile-face-bd29d51b tile-back-bd29d51b"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-bd29d51b")){return;}
var back=tile.querySelector(".tile-back-bd29d51b");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-bd29d51b");if(isMine){tile.classList.add("boom-bd29d51b");}
if(dim){tile.classList.add("dimmed-bd29d51b");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-bd29d51b")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-bd29d51b");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-bd29d51b")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-bd29d51b");void panelEl.offsetWidth;panelEl.classList.add("jolt-bd29d51b");}
endRound();}
